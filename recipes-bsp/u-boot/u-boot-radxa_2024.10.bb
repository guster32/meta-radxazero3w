...
# --- pylibfdt compile fix (TODO / DEFERRED) -------------------------------
# u-boot v2024.10's scripts/dtc/pylibfdt/libfdt_wrap.c shipped with the
# source tree was generated by an older SWIG (4.0.x) that emits calls to
# SWIG_Python_AppendOutput with two args (fmt, value). wrynose 6.0 builds
# SWIG 4.4.1 which declares SWIG_Python_AppendOutput with three args:
#   (PyObject *result, const char *fmt, ...).
# Hence wrynose's host gcc sees a mismatch: too few arguments.
#
# Two paths to a fix:
#   (a) bbappend swig-native in OE-core to pin to swig 4.0/4.1 (OE-core
#       maintenance territory, not arcadia-owned).
#   (b) Bump this recipe to u-boot v2026.01 (wrynose's oe-core default
#       version), whose pylibfdt.wrap is regenerated with the newer SWIG.
#
# Until then, this recipe accepts `do_compile` will fail at scripts/dtc/
# pylibfdt/libfdt_wrap.c. Track in scripts/README.md as a known-pending
# item for the next run; full image builds fail at the same location.
#
# Do NOT disable the pylibfdt subdir: binman / dtoc / libfdt Python all
# depend on it, and dtoc/fdt.py imports `from libfdt import QUIET_NOTFOUND`.
# A11f the warning has been removed and the pylibfdt submodule is enabled.
